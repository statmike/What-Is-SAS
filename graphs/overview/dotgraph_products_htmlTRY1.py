...
actions_input = myreader('crawlers/actions_by_product/','actions',header='drop')
...
for edge in prodedge_input:
    ...
    #else: from_prod = next((item[7] for item in procs_linked_input if item[0] == edge[0]),'Nothing')
    if edge[1] == 'SAS Visual Analytics': to_prod = 'SVA'
    elif edge[1] == 'SAS IML': to_prod = 'SIML'
    else: to_prod = clusters[edge[1]+'_NP']
    #else: to_prod = next((item[7] for item in procs_linked_input if item[0] == edge[1]),'Nothing')
